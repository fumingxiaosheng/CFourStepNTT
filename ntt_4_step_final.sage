...
def inverse_ntt(src, n, q, inv_root, is_4step=False):
    ...
    step = 1
    while(step < n):
        for tid in range(n >> 1):#代表的是第tid个线程
            len = int(n / step / 2) #分为len个组 
            psi_step = int(tid / step) #psi_step代表位于具体哪个组里
            tar_idx = psi_step * step * 2 + (tid % step) #开始的位置
            step_group = len + psi_step #组数加上位于哪一个组
