...
__global__ void FourStepPartialInverseCore(Data* polynomial_in, Data* polynomial_out, Root* n2_root_of_unity_table,
                                           ...
                                           int poly_n_power, int mod_count); //HXW修改函数声明
...
//增加一个constant内存，用于存储旋转因子

//__constant__ Root C_psitable64[32]; //64维共有32个旋转因子
//extern __constant__ Root Csitable64[32]; //在这里定义可能会引起其他文件找不到声明的外部变量导致编译出错