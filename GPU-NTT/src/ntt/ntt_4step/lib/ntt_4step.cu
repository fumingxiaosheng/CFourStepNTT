...
/*2024-8-19:
完成64维NTT操作
参数:polynominal_in:n2*n1维
    polynominal_out: n1*n2维度
    index1 =n2  index2 = n2+1 index3 = 1 << (n2+4)
线程组织结构:<<<dim3(n/1024，batch_size),dim3(32, 8)>>>*/
                                      ...
                                      int index2, int index3, int n_power, int mod_count)
{
    ...
    //32个线程完成一个64维NTT,每个idx_y负责横着的idx_y和idx_y+8行的蝴蝶操作
...
/*2024-9-20:
增大共享内存，使得隐藏矩阵转置的开销
线程组织形式: <<<dim3(32,16)>>> //TODO:线程组织形式，根据占用率做进一步的优化

index1 =n2  index2 = n2+1 index3 = 1 << (n2+4) ->扩大倍数
*/
/*__global__ void cyclic_6_v1_not_done(Data* polynomial_in, Data* polynomial_out,
                                      Root* n1_root_of_unity_table, Modulus* modulus, int index1,
                                      int index2, int index3, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
    ...
    __shared__ Data sharedmemorys[32][64 + 1];
    ...
    // Load data from global & store to shared
    sharedmemorys[idx_x][idx_y] = polynomial_in[(idx_y << index1) + idx_x + (block_x << 5) + divindex];
    sharedmemorys[idx_x][idx_y + 16] = polynomial_in[((idx_y + 16) << index1) + idx_x + (block_x << 5) + divindex];
    sharedmemorys[idx_x][idx_y + 32] = polynomial_in[((idx_y + 32) << index1) + idx_x + (block_x << 5) + divindex];
    sharedmemorys[idx_x][idx_y + 48] = polynomial_in[((idx_y + 48) << index1) + idx_x + (block_x << 5) + divindex];
    __syncthreads();
    ...
    CooleyTukeyUnit_(sharedmemorys[idx_y][in_shared_address],
                    sharedmemorys[idx_y][in_shared_address + t],
                    n1_root_of_unity_table[(idx_x >> t_)], q_thread);
    CooleyTukeyUnit_(sharedmemorys[idx_y + 16][in_shared_address],
                    sharedmemorys[idx_y + 16][in_shared_address + t],
                    n1_root_of_unity_table[(idx_x >> t_)], q_thread);
    __syncthreads();
...
__global__ void cyclic_6_nmt(Data* polynomial_in, Data* polynomial_out,
                                      Root* n1_root_of_unity_table, Modulus* modulus, int index1,
                                      int index2, int index3, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
    ...
    polynomial_out[((idx_y + (global_index1 << 4)) << index1) + global_index2 + (block_x << 4) + divindex] = sharedmemorys[idx_y / 2][idx_x + ((idx_y & 1) << 5)];
    polynomial_out[((idx_y + (global_index1 << 4) + 8) << index1) + global_index2 + (block_x << 4) + divindex] = sharedmemorys[idx_y / 2 +4][idx_x + ((idx_y & 1) << 5)];
    polynomial_out[((idx_y + (global_index1 << 4) + 32) << index1) + global_index2 + (block_x << 4) + divindex] = sharedmemorys[idx_y / 2 +8][idx_x + ((idx_y & 1) << 5)];
    polynomial_out[((idx_y + (global_index1 << 4) + 40) << index1) + global_index2 + (block_x << 4) + divindex] = sharedmemorys[idx_y / 2 +12][idx_x + ((idx_y & 1) << 5)];
}


/*2024-9-20:
根据16*64的共享内存特性，完成矩阵转置的操作,注意,该方法无法形成对于全局内存的合并内存访问，只能形成1/2
线程组织形式为<<<,dim3(32,8)>>>
index1 =n2  index2 = n2+1 index3 = 1 << (n2+4)
TODO:在做蝴蝶变换的时候,仍然会引起存储体冲突，这是需要解决的->这是由于存储体冲突引起的 ->能不能变换一下*/
__global__ void cyclic_6(Data* polynomial_in, Data* polynomial_out,
                                      Root* n1_root_of_unity_table, Modulus* modulus, int index1,
                                      int index2, int index3, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
...
__global__ void cyclic_7_nmt(Data* polynomial_in, Data* polynomial_out,
                                      Root* n1_root_of_unity_table, Modulus* modulus, int index1,
                                      int index2, int index3, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
    ...
    polynomial_out[(((global_index1 << 3) + idx_y ) << index1) + global_index2 + (block_x << 3) + divindex] = sharedmemorys[idx_y/4][idx_x + 32 * (idx_y &3)];
    polynomial_out[(((global_index1 << 3) + idx_y + 32) << index1) + global_index2 + (block_x << 3) + divindex] = sharedmemorys[idx_y/4 + 2][idx_x + 32 * (idx_y &3)];
    polynomial_out[(((global_index1 << 3) + idx_y + 64) << index1) + global_index2 + (block_x << 3) + divindex] = sharedmemorys[idx_y/4 + 4][idx_x + 32 * (idx_y &3)];
    polynomial_out[(((global_index1 << 3) + idx_y + 96) << index1) + global_index2 + (block_x << 3) + divindex] = sharedmemorys[idx_y/4 + 6][idx_x + 32 * (idx_y &3)];
...
参数:
    ...
    index3 = 1 << (n2 + 6)
*/
__global__ void cyclic_8(Data* polynomial_in, Data* polynomial_out,
                                      Root* n1_root_of_unity_table, Modulus* modulus, int index1,
                                      int index2, int index3, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
...
__global__ void cyclic_8_mnt(Data* polynomial_in, Data* polynomial_out,
                                      Root* n1_root_of_unity_table, Modulus* modulus, int index1,
                                      int index2, int index3, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
    ...
    sharedmemorys[0][idx_x + idx_y * 32] = polynomial_in[(((global_index1 << 2) + ((idx_y >> 2) << 5) + (idx_y & 3)) << index1) + (block_x << 2) + global_index2 + divindex];
    sharedmemorys[1][idx_x + idx_y * 32] = polynomial_in[(((global_index1 << 2) + ((idx_y >> 2) << 5) + (idx_y & 3) + 64) << index1) + (block_x << 2) + global_index2 + divindex];
    sharedmemorys[2][idx_x + idx_y * 32] = polynomial_in[(((global_index1 << 2) + ((idx_y >> 2) << 5) + (idx_y & 3) + 128) << index1) + (block_x << 2) + global_index2 + divindex];
    sharedmemorys[3][idx_x + idx_y * 32] = polynomial_in[(((global_index1 << 2) + ((idx_y >> 2) << 5) + (idx_y & 3) + 192) << index1) + (block_x << 2) + global_index2 + divindex];
    __syncthreads();
    ...
    polynomial_out[(((global_index1 << 2) + ((idx_y >> 2) << 5) + (idx_y & 3)) << index1) + (block_x << 2) + global_index2 + divindex] = sharedmemorys[0][idx_x + idx_y * 32];
    polynomial_out[(((global_index1 << 2) + ((idx_y >> 2) << 5) + (idx_y & 3) + 64) << index1) + (block_x << 2) + global_index2 + divindex] = sharedmemorys[1][idx_x + idx_y * 32];
    polynomial_out[(((global_index1 << 2) + ((idx_y >> 2) << 5) + (idx_y & 3) + 128) << index1) + (block_x << 2) + global_index2 + divindex] = sharedmemorys[2][idx_x + idx_y * 32];
    polynomial_out[(((global_index1 << 2) + ((idx_y >> 2) << 5) + (idx_y & 3) + 192) << index1) + (block_x << 2) + global_index2 + divindex] = sharedmemorys[3][idx_x + idx_y * 32];
}

/*2024-10-4:
线程组织结构:<<<dim3(n/1024),dim3(32,8)>>>
参数:
    n_power:log(n)*/
__global__ void cyclic_8_nm(Data* polynomial_in, Data* polynomial_out, Root* n2_root_of_unity_table, Root* w_root_of_unity_table, Modulus* modulus, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
    int idx_y = threadIdx.y;
    int block_x = blockIdx.x;
    ...
    sharedmemorys[0][(idx_y << 5) + idx_x] = VALUE_GPU::mult(polynomial_in[(idx_y << 5) + idx_x + (0 << 8) + (block_x << 10) + divindex],w_root_of_unity_table[(idx_y << 5) + idx_x + (0 << 8) + (block_x << 10)], q_thread);
    sharedmemorys[1][(idx_y << 5) + idx_x] = VALUE_GPU::mult(polynomial_in[(idx_y << 5) + idx_x + (1 << 8) + (block_x << 10) + divindex],w_root_of_unity_table[(idx_y << 5) + idx_x + (1 << 8) + (block_x << 10)], q_thread);
    sharedmemorys[2][(idx_y << 5) + idx_x] = VALUE_GPU::mult(polynomial_in[(idx_y << 5) + idx_x + (2 << 8) + (block_x << 10) + divindex],w_root_of_unity_table[(idx_y << 5) + idx_x + (2 << 8) + (block_x << 10)], q_thread);
    sharedmemorys[3][(idx_y << 5) + idx_x] = VALUE_GPU::mult(polynomial_in[(idx_y << 5) + idx_x + (3 << 8) + (block_x << 10) + divindex],w_root_of_unity_table[(idx_y << 5) + idx_x + (3 << 8) + (block_x << 10)], q_thread);
    __syncthreads();
    ...
    CooleyTukeyUnit_(sharedmemorys[shr_in][in_shared_address],
                    sharedmemorys[shr_in][in_shared_address + t],
                    n2_root_of_unity_table[(shr_address >> t_)], q_thread);
    CooleyTukeyUnit_(sharedmemorys[shr_in + 2][in_shared_address],
                    sharedmemorys[shr_in + 2][in_shared_address + t],
                    n2_root_of_unity_table[(shr_address >> t_)], q_thread);
    __syncthreads();
    ...
    polynomial_out[(idx_y << 5) + idx_x + (0 << 8) + (block_x << 10) + divindex] = sharedmemorys[0][(idx_y << 5) + idx_x];
    polynomial_out[(idx_y << 5) + idx_x + (1 << 8) + (block_x << 10) + divindex] = sharedmemorys[1][(idx_y << 5) + idx_x];
    polynomial_out[(idx_y << 5) + idx_x + (2 << 8) + (block_x << 10) + divindex] = sharedmemorys[2][(idx_y << 5) + idx_x];
    polynomial_out[(idx_y << 5) + idx_x + (3 << 8) + (block_x << 10) + divindex] = sharedmemorys[3][(idx_y << 5) + idx_x];
}
...
/*2024-10-3:
线程组织形式<<<dim3((1 << n2)/512,1 << n1),,dim3(256/(1<<(层数-1)),1<<(层数-1))>>>
参数：
    small_power:n2
    n_power:n
    loc2:第一次读取时的间隔 1<< (n2-1)
    loop:需要完成的循环次数
    loc1:log(256/(1<<(层数-1)) = blockIdx.x)
...
__global__ void cyclic_5_nmt(Data* polynomial_in, Data* polynomial_out,Root* n1_root_of_unity_table, Modulus* modulus, int index1, int index2, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
    int idx_y = threadIdx.y;
    int block_x = blockIdx.x;
    ...
    //每一个共享内存的一行,对应着在多项式中连续的32个系数
    sharedmemorys[idx_y][idx_x] = polynomial_in[idx_x + (idx_y << index1) + (block_x << 5) + divindex];
    sharedmemorys[idx_y + 8][idx_x] = polynomial_in[idx_x + (idx_y << index1) + (block_x << 5) + index2 + divindex];
    sharedmemorys[idx_y + 16][idx_x] = polynomial_in[idx_x + (idx_y << index1) + (block_x << 5) + (index2 * 2) + divindex];
    sharedmemorys[idx_y + 24][idx_x] = polynomial_in[idx_x + (idx_y << index1) + (block_x << 5) + (index2 * 3) + divindex];
    __syncthreads();
...
__global__ void cyclic_5(Data* polynomial_in, Data* polynomial_out,Root* n1_root_of_unity_table, Modulus* modulus, int index1, int index2, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
    int idx_y = threadIdx.y;
    int block_x = blockIdx.x;
...
/*2024-9-22:
32维循环卷积逆NTT*/
__global__ void cyclic_inv_5(Data* polynomial_in, Data* polynomial_out,Root* n1_root_of_unity_table, Modulus* modulus, int index1, int index2, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
    int idx_y = threadIdx.y;
    int block_x = blockIdx.x;
...
/*2024-9-22:*/
__global__ void cyclic_inv_7(Data* polynomial_in, Data* polynomial_out,
                                      Root* n1_root_of_unity_table, Modulus* modulus, int index1,
                                      int index2, int index3, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
...
*/
__global__ void FourStepPartialInverseCore_v1(Data* polynomial_in, Data* polynomial_out,Root* n2_root_of_unity_table,
                                           Root* w_root_of_unity_table, Modulus* modulus,
                                           ...
                                           int poly_n_power, int mod_count)
{
    ...
    int dividx = (block_x << small_npower_); //n2 * block_x

    int divindex = block_y << poly_n_power; //偏移batch * n

    int q_index = block_y % mod_count;
    Modulus q_thread = modulus[q_index];

    int address = dividx + ((local_idx >> t_2) << t_2) + local_idx;//等于dividx + 2 * local_idx
...
#pragma unroll
    ...
    Data temp1 = sharedmemorys[shrd_address];
    Data temp2 = sharedmemorys[shrd_address + t];
    GentlemanSandeUnit_(temp1, temp2,
                       n2_root_of_unity_table[(local_idx >> t_2)], q_thread);
    __syncthreads();

    temp1 = VALUE_GPU::mult(temp1, inverse[0], q_thread); //除以n
    polynomial_out[address + divindex] = VALUE_GPU::mult(temp1 , w_root_of_unity_table[address], q_thread); //乘上修正因子

    temp2 = VALUE_GPU::mult(temp2, inverse[0], q_thread);
    polynomial_out[address + t + divindex] = VALUE_GPU::mult(temp2, w_root_of_unity_table[address + t], q_thread); //乘上修正因子
}
                                            ...
                                            int poly_n_power, int mod_count)
{
    ...
    polynomial_out[load_store_address + divindex] = VALUE_GPU::mult(temp1,w_root_of_unity_table[load_store_address], q_thread);
    ...
    polynomial_out[load_store_address + loc2 + divindex] = VALUE_GPU::mult(temp2,w_root_of_unity_table[load_store_address + loc2], q_thread);
}
        ...
        case FORWARD:
            ...
            switch(cfg.n_power)
            {
                case 12:
                    ...
                    BEFORE_SPEED
                    //GPU_Transpose(device_out, device_in, 1 << 5, 1 << 7, 12, batch_size);
                ...
                case 13:
                    ...
                    //printf("compute 13\n");
                    BEFORE_SPEED
                    cyclic_5<<<dim3(8, batch_size), dim3(32, 8)>>>(
                    ...
                    //cyclic_8_nm<<<dim3(8,batch_size),dim3(32,8)>>>(device_out, device_out,n2_root_of_unity_table,W_root_of_unity_table,modulus,13,mod_count); //注意这句代码是不用的
                    // AFTER_SPEED
                    // tot = timer(start,stop);
                    // DESTORY_SPEED
                    // printf("%f\n",tot);
                ...
                case 14:
                    ...
                    //6+8
                    BEFORE_SPEED
                    //GPU_Transpose(device_in, device_out, 1 << 6, 1 << 8, 14, batch_size);
                ...
                case 16:
                    ...
                    //8+8
                    // BEFORE_SPEED
                    // cyclic_8<<<dim3(64,batch_size),dim3(32,8)>>>(device_in,device_out,n1_root_of_unity_table,modulus,8,11,16384,16,mod_count);
                    // FourStepPartialForwardCore<<<dim3(256,batch_size),128>>>(device_out,n2_root_of_unity_table,W_root_of_unity_table,modulus,8,7,2,16,mod_count);
                    // AFTER_SPEED
                    // tot = timer(start,stop);
                    // DESTORY_SPEED
                    // printf("%f\n",tot);
                ...
                case 17:
                    ...
                    // cyclic_5<<<dim3(128, batch_size), dim3(32, 8)>>>(
                    //     device_in, device_out, n1_root_of_unity_table, modulus, 12, 32768, 17,
                    //     mod_count);//device_in:n2*n1维 device_out:n1*n2维
                    // THROW_IF_CUDA_ERROR(cudaGetLastError());
                    ...
                    //8+9
                    BEFORE_SPEED
                    cyclic_8<<<dim3(128, batch_size), dim3(32, 8)>>>(device_in, device_out, n1_root_of_unity_table, modulus, 9, 12, 32768, 17, mod_count);//device_in:n2*n1维 device_out:n1*n2维
                    AFTER_SPEED
                    tot = timer(start,stop);
                    DESTORY_SPEED
                    printf("%f\n",tot);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    //BEFORE_SPEED
                    FourStepPartialForwardCore<<<dim3(256, batch_size), 256>>>(device_out, n2_root_of_unity_table, W_root_of_unity_table, modulus, 9, 8, 3, 17, mod_count);
                    // AFTER_SPEED
                    // tot = timer(start,stop);
                    // DESTORY_SPEED
                    // printf("%f\n",tot);
                ...
                case 19:
                    ...
                    //printf("compute 19\n");
                    BEFORE_SPEED
                    //GPU_Transpose(device_in, device_out, 1 << 5, 1 << 14, 19, batch_size);
                    cyclic_5<<<dim3(512, batch_size), dim3(32, 8)>>>(
                ...
                case 20:
                    // 5 + 15
                    //printf("compute 20\n");
                    cyclic_5<<<dim3(1024, batch_size), dim3(32, 8)>>>(
                ...
                case 21:
                    ...
                    //printf("compute 21\n");
                    BEFORE_SPEED
                    //GPU_Transpose(device_in, device_out, 1 << 6, 1 << 15, 21, batch_size);
                ...
                case 22:
                    ...
                    //printf("compute 22\n");
                    BEFORE_SPEED
                    //GPU_Transpose(device_in, device_out, 1 << 7, 1 << 15, 22, batch_size);
                ...
                case 24:
                    // 8 + 16
                    //printf("compute 24 new\n");
                    BEFORE_SPEED
                    //GPU_Transpose(device_in, device_out, 1 << 8, 1 << 16, 24, batch_size);
                    cyclic_8<<<dim3(16384, batch_size), dim3(32, 8)>>>(
        ...
        case INVERSE:
            switch(cfg.n_power)
            {
                case 12:
                    ...
                    cyclic_inv_5<<<dim3(4, batch_size), dim3(32, 8)>>>(
                ...
                case 13:
                    ...
                    cyclic_inv_5<<<dim3(8, batch_size), dim3(32, 8)>>>(
                ...
                case 14:
                    ...
                    cyclic_inv_5<<<dim3(16, batch_size), dim3(32, 8)>>>(
                ...
                case 17:
                    ...
                    cyclic_inv_5<<<dim3(128, batch_size), dim3(32, 8)>>>(
                ...
                case 18:
                    ...
                    cyclic_inv_5<<<dim3(256, batch_size), dim3(32, 8)>>>(
                ...
                case 19:
                    ...
                    cyclic_inv_5<<<dim3(512, batch_size), dim3(32, 8)>>>(
                ...
                case 20:
                    ...
                    cyclic_inv_5<<<dim3(1024, batch_size), dim3(32, 8)>>>(
                ...
                case 21:
                    ...
                    cyclic_inv_6<<<dim3(2048, batch_size), dim3(32, 8)>>>(
                ...
                case 22:
                    ...
                    cyclic_inv_7<<<dim3(4096, batch_size), dim3(32, 8)>>>(
                ...
                case 24:
                    ...
                    cyclic_inv_8<<<dim3(16384, batch_size), dim3(32, 8)>>>(
        ...
        case FORWARD:
            switch(cfg.n_power)
            {
                case 12:
                    //32 * 128
                    FourStepForwardCoreT1<<<dim3(4, batch_size), dim3(32, 8)>>>(
                        device_in, device_out, n1_root_of_unity_table, modulus, 7, 1024, 12);//128个32维度ntt
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    FourStepPartialForwardCore<<<dim3(32, batch_size), 64>>>(
                        device_out, n2_root_of_unity_table, W_root_of_unity_table, modulus, 7, 6, 1,
                        12);
...
负循环卷积32维的高效实现
参数:
polynomial_in:n1*n2维
polynomial_out:n1*n2维
index1: log(n2)
index2: n2 * 8

<<<dim3(4, batch_size), dim3(32, 8)>>>
*/
__global__ void negative_5(Data* polynomial_in, Data* polynomial_out,Root* n1_root_of_unity_table, Modulus* modulus, int index1, int index2, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
    int idx_y = threadIdx.y;
    int block_x = blockIdx.x;
    ...
    int t_ = 4;
    int t = 1 << t_;
    int in_shared_address = ((global_index2 >> t_) << t_) + global_index2; //= ((global_index2 >> t_) << (t_ + 1)) + global_index % (1 << t_)

    //NTT操作:in_shared_address对应着矩阵sharedmemorys的一列，该列需要完成32个CT操作。in_shared_address的范围为0-16，因此两个idx_x映射到同一个列。因此global_index2对应了哪一列，global_index1对应着该idx_x是处理奇数行还是偶数行（因此行位置为(idx_y << 1) + global_index1，又因为每一列16个线程，因此每个线程做两个CT，再加上16），(global_index2 >> t_代表着做的是第几组CT，又因为n1_root_of_unity_table已经是比特翻转顺序，因此直接索引其下标即可
    CooleyTukeyUnit_(sharedmemorys[(idx_y << 1) + global_index1][in_shared_address],
                    sharedmemorys[(idx_y << 1) + global_index1][in_shared_address + t],
                    n1_root_of_unity_table[1 + (global_index2 >> t_)], q_thread);
    CooleyTukeyUnit_(sharedmemorys[(idx_y << 1) + global_index1 + 16][in_shared_address],
                    sharedmemorys[(idx_y << 1) + global_index1 + 16][in_shared_address + t],
                    n1_root_of_unity_table[1 + (global_index2 >> t_)], q_thread);
    __syncthreads();
...
完成负循环卷积NTT
*/
__global__ void negative_6(Data* polynomial_in, Data* polynomial_out, Root* n1_root_of_unity_table, Modulus* modulus, int index1, int index2, int index3, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
    int idx_y = threadIdx.y;
    int block_x = blockIdx.x;
    ...
    CooleyTukeyUnit_(sharedmemorys[idx_y][in_shared_address],
                    sharedmemorys[idx_y][in_shared_address + t],
                    n1_root_of_unity_table[1 + (idx_x >> t_)], q_thread);
    CooleyTukeyUnit_(sharedmemorys[idx_y + 8][in_shared_address],
                    sharedmemorys[idx_y + 8][in_shared_address + t],
                    n1_root_of_unity_table[1 + (idx_x >> t_)], q_thread);
    __syncthreads();
...
/*2024-9-21:
n=7的负循环卷积*/
__global__ void negative_7(Data* polynomial_in, Data* polynomial_out,
                                      Root* n1_root_of_unity_table, Modulus* modulus, int index1,
                                      int index2, int index3, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
    ...
    CooleyTukeyUnit_(sharedmemorys[shr_in][in_shared_address],
                    sharedmemorys[shr_in][in_shared_address + t],
                    n1_root_of_unity_table[1 + (shr_address >> t_)], q_thread);
    CooleyTukeyUnit_(sharedmemorys[shr_in + 4][in_shared_address],
                    sharedmemorys[shr_in + 4][in_shared_address + t],
                    n1_root_of_unity_table[1 + (shr_address >> t_)], q_thread);
    __syncthreads();
...
/*2024-9-22:
完成256维的负循环卷积操作*/
__global__ void negative_8(Data* polynomial_in, Data* polynomial_out,
                                      Root* n1_root_of_unity_table, Modulus* modulus, int index1,
                                      int index2, int index3, int n_power, int mod_count)
{
    int idx_x = threadIdx.x;
        ...
        case FORWARD:
            switch(cfg.n_power){
                case 12:
                    //5+7
                    //printf("compute 12\n");
                    negative_5<<<dim3(4, batch_size), dim3(32, 8)>>>(device_in, device_out, negative_2n1_root_of_unity_table, modulus, 7, 1024, 12, mod_count); //n2个n1维的NTT
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    FourStepPartialForwardCore<<<dim3(32, batch_size), 64>>>(device_out, negative_n2_root_of_unity_table, negative_W_root_of_unity_table, modulus, 7, 6, 1,12, mod_count);//n1个n2维的NTT ,device_out为n1*n2维的矩阵
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                
                case 13:
                    //printf("compute 13\n");
                    negative_5<<<dim3(8, batch_size), dim3(32, 8)>>>(device_in, device_out, negative_2n1_root_of_unity_table, modulus, 8, 2048, 13,mod_count);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    FourStepPartialForwardCore<<<dim3(32, batch_size), 128>>>(device_out, negative_n2_root_of_unity_table, negative_W_root_of_unity_table, modulus, 8, 7, 2, 13, mod_count);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 14:
                    //printf("compute 14\n");
                    negative_5<<<dim3(16, batch_size), dim3(32, 8)>>>(device_in, device_out, negative_2n1_root_of_unity_table, modulus, 9, 4096, 14, mod_count);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                ...
                case 15:
                    ...
                    negative_6<<<dim3(32, batch_size), dim3(32, 8)>>>(device_in, device_out, negative_2n1_root_of_unity_table, modulus, 9, 10, 8192, 15, mod_count);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    FourStepPartialForwardCore<<<dim3(64, batch_size), 256>>>(device_out, negative_n2_root_of_unity_table, negative_W_root_of_unity_table, modulus, 9, 8, 3, 15, mod_count);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                ...
                case 19:
                    // 5 + 14
                    //printf("compute 19\n");
                    negative_5<<<dim3(512, batch_size), dim3(32, 8)>>>(device_in, device_out, negative_2n1_root_of_unity_table, modulus, 14, 131072, 19, mod_count);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
